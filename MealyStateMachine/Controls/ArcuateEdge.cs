...
public class ArcuateEdge : Edge
{
	private PointF[] _archPoints = new PointF[2];

	public ArcuateEdge(Node sourceNode, Node destNode, string text = "")
	...
	public override void Adjust()
	{
		...
		_archPoints[0] = _sourcePoint.CalculatеPerpendicularPoint(
			_sourcePoint.Center(center), GraphicsDrawable.Unit * 4, len, true);
		_archPoints[1] = _destPoint.CalculatеPerpendicularPoint(
			_destPoint.Center(center), GraphicsDrawable.Unit * 4, len, false);
		...
		_textPoint = _sourcePoint.CalculatеPerpendicularPoint(
			center, GraphicsDrawable.Unit * 2, len);
